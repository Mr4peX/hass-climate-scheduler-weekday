...
CONF_SCHEDULE_WEEKDAYS = "weekdays"
    ...
    [
        {
            ...
            vol.Optional(CONF_SCHEDULE_WEEKDAYS, default=[1, 2, 3, 4, 5, 6, 7]): vol.All(
                cv.ensure_list,
...
class ClimateShedulerSchedule:
    def __init__(self, config: dict) -> None:
        ...
        self._weekdays: List[int] = config.get(CONF_SCHEDULE_WEEKDAYS)
    ...
    def applies_to_weekday(self, weekday: int) -> bool:
        """Check if this schedule applies to given weekday (1=Mon, 7=Sun)"""
        return weekday in self._weekdays
...
class ClimateSchedulerProfile:
    ...
    def compute_climate(self, time_of_day: timedelta, current_weekday: int) -> ComputedClimateData:
        schedule = self._find_schedule(time_of_day, current_weekday)
        if schedule is None:
    ...
    def _find_schedule(
        self, time_of_day: timedelta, current_weekday: int
    ) -> Optional[ClimateShedulerSchedule]:
        if len(self._schedules) == 0:
            return None

        # Filter schedules by current weekday
        applicable_schedules = [s for s in self._schedules if s.applies_to_weekday(current_weekday)]

        if len(applicable_schedules) == 0:
            return None

        if len(applicable_schedules) == 1:
            return applicable_schedules[0]

        # If the current time is earlier than the first schedule, wrap around and
        # return the last schedule of the day
        if time_of_day < applicable_schedules[0].time:
            return applicable_schedules[-1]

        for index, schedule in enumerate(applicable_schedules):
            ...
            next_schedule = None
            if index < len(applicable_schedules) - 1:
                next_schedule = applicable_schedules[index + 1]
...
class ClimateSchedulerSwitch(SwitchEntity, RestoreEntity):
    ...
    async def async_update_climate(self, *args, **kwargs) -> None:
        ...
        current_weekday = dt.isoweekday()  # 1=Monday, 7=Sunday
        climate_data = self._current_profile.compute_climate(time_of_day, current_weekday)
